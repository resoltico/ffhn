...
3. dry-run affected targets with `cargo run -p ffhn-cli -- run --watch-root <root> --target <id> --dry-run`
4. if the change affects live persistence, run a live target and inspect `cargo run -p ffhn-cli -- status --watch-root <root> --target <id>`
...
2. target schema, typed-value parameters, or examples: [docs/targets.md](docs/targets.md), [examples/](examples), and [watchlist/demo/target.toml](watchlist/demo/target.toml)
3. runtime, persistence, or FFHN-owned contract semantics such as `ffhn.target`, `ffhn.state`, `ffhn.run_report`, `ffhn.batch_run_report`, `ffhn.status_report`, or `ffhn.reset_report`: [docs/core.md](docs/core.md), [docs/reports.md](docs/reports.md), [docs/run-reports.md](docs/run-reports.md), and [docs/contracts.md](docs/contracts.md)
...
2. persisted sample data when `ffhn.state`, `ffhn.run_report`, `ffhn.batch_run_report`, `ffhn.status_report`, or `ffhn.reset_report` semantics change
...
`watchlist/demo` is maintained as starter target configuration, not as a checked-in runtime artifact. The file-target example under [examples/file-target-json](examples/file-target-json) materializes an absolute `file_path`. Generated `.ffhn/` storage and `.ffhn-locks/` lock files are local runtime artifacts.
...
2. `ffhn.state`, `ffhn.run_report`, `ffhn.batch_run_report`, `ffhn.status_report`, or `ffhn.reset_report`
3. file-target or JSON Pointer acquisition behavior
