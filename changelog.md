...
- Added pinned cargo-mutants testing across separate runtime and maintainer-tool scopes, with copied-workspace local execution, isolated per-worker Cargo artifact roots, diff-scoped pull-request checks, sixteen exact scheduled/manual shards, artifact-safe identities, retained bounded evidence, and fail-closed aggregation of the complete outcome set.
- Added the FFHN observation graph: one `ffhn.agent` graph root owns independently configured `ffhn.source` acquisitions, and each source owns any number of `ffhn.measurement` typed scalar projections over one complete shared representation acquired once per cycle.
- Added a singleton fixed-interval agent with graph and source advisory locks, real source-level `--jobs` concurrency through a continuously fed bounded worker queue, stable source-ordered reports, finite `agent tick`, continuous `agent run`, graph-wide `agent status`, and interruptible SIGINT/SIGTERM shutdown that finishes already-started source turns.
- Added independently paced acquisition, source-outbox, and measurement-outbox capabilities. Each withdrawn capability retains a closed deferral reason and UTC boundary, scheduling incorporates durable due and retry times, and a failure in one source cannot stop other sources or create an unpaced retry loop.
- Added random UUIDv4 graph, source, and measurement lineage with `.ffhn-identity.json` as the sole identity authority. The first accepted projection installs measurement identity and state atomically, configuration removal never silently prunes lineage or queued delivery, and source- or measurement-scoped reset always mints non-reusing lineage.
- Added capability-scoped, no-follow graph storage with crash-recoverable `ffhn.lineage_manifest` and `ffhn.commit_manifest` protocols, content-hash prior/result guards, atomic per-source generations, fixed tombstone ownership, idempotent recovery, and scope-correct refusal of foreign, unreadable, missing, or misplaced lineage-dependent artifacts.
- Added source acquisition health and measurement extraction health as separate durable lifecycles, code-keyed integration-fault episodes at each ownership boundary, route-independent lifecycle and condition event envelopes, durable bounded outbox-overflow evidence, and status reports that expose configuration, lineage, manifests, quarantine, health, faults, digests, observations, and scheduling facts without requiring delivery routes.
- Added direct conditional HTTP acquisition with provenance-bound `ETag` and `Last-Modified` validators. A direct matching `304` is `not_modified`, redirected validators are never reused at another URL, and FFHN forces a complete document whenever an eligible measurement has no accepted observation.
- Added environment-backed acquisition and webhook header secrets whose values are resolved only for the current attempt and are never persisted, logged, echoed, or hashed. Secret revision identifiers participate in source representation identity, while durable delivery snapshots retain only environment references and output formats.
- Added snapshot-only HTTPS webhook delivery alongside process-stdin delivery with deadline-bounded process-tree cleanup, stdin writes, and stderr collection; deterministic symmetric retry jitter; append-only attempt histories; atomic delivery-result generations; durable dead letters; status-only webhook acknowledgement with bounded response-body discard; and continued draining after configuration, route, origin, or measurement removal when the stored lineage remains authoritative.
- Added offline graph `validate` and `list`, `new source` and `new measurement` configuration scaffolding that initializes graph identity when needed but never source or measurement lineage, source/measurement-scoped `status` and `reset`, live and dry `measure`, version-1 graph-native operation reports, and graph-native fuzz harnesses for configuration, durable documents, and dry-run file acquisition.

### Changed

- Replaced the v10 target/watch-root runtime, schemas, commands, examples, reports, state, delivery facade, and public Rust exports with the graph/source/measurement model as a clean 11.0.0 break. Every current FFHN-owned graph document starts at schema version 1; there is no compatibility reader, migration command, shim, alias, or retained legacy path.
- Split source representation identity, measurement value identity, and condition-definition identity. The SRD contains only representation-affecting source facts, the MVD composes the SRD with projection, declared type, type parameters, and explicit parser/acquisition/HTMLCut semantics, and normalized per-condition digests let policy-only edits rebase independently without discarding accepted observations or immutable queued delivery snapshots.
- Changed HTTP acquisition to accept only complete `200` and `203` representations, classify every other status and transport outcome through closed typed evidence, distinguish connect, read-idle, and total timeouts from framing, limit, redirect, and decode failures, and retain both the configured source URL and effective HTTP base needed to audit validators and HTML rendering.
- Changed local-file acquisition to read bounded UTF-8 bytes every cycle, use a fixed SHA-256 content digest for an already-initialized source when no eligible measurement requires a document, and project a fresh document whenever any eligible measurement lacks an accepted observation.
- Changed event identity to bind immutable graph/source/measurement lineage plus typed condition-definition, observation-sequence, or episode-sequence facts while excluding routes and wall-clock time. Emitter-local outbox admission preserves lifecycle/condition-issue priority and declaration order, never evicts an existing pending record, and reports every rejected admission; reset completion is report-only and emits no delivery event.
- Changed reset from blind deletion of target state to a mint-only lineage transition. Source reset replaces all source-owned state and queued delivery under fresh source lineage, measurement reset replaces only the selected measurement while preserving source and sibling lineage, and neither scope interprets old artifacts as migration input.
- Changed reports and CLI failure handling so handled acquisition, projection, configuration, lineage, manifest, quarantine, integration, delivery, and status failures retain structured stdout and exit 1. Manual source contention and graph-agent contention exit 4, while scheduled contention is a successful `skipped_locked` scheduling fact.
- Changed dry measurement to execute the real configuration, lineage, acquisition, projection, parsing, and policy path under a shared source lock while never recovering manifests, minting lineage, writing state, admitting outbox records, delivering payloads, or materializing event identities from transient authority.
- Refactored `ffhn-core` around the observation-graph ownership boundary and a small shared typed-observation/exact-policy kernel, deleted the obsolete v2 runtime/model/report/contract facade rather than retaining compatibility layers, and reworked `ffhn-cli`, fuzzing, tests, source-shape policy, and contributor guidance around the same domain language.
- Reauthored the public documentation around graph, source, and measurement ownership; removed the obsolete target examples, starter watchlist, and retired report guide; and documented the supported non-notarized macOS installation boundary without prescribing a quarantine bypass.
- Hardened the maintainer release protocol's local-branch cleanup: when GitHub removes a merged PR's remote branch, the safe local fallback now requires proof that the branch is already contained in `origin/main`, preventing both stale release debris and blind force-deletes.
- Raised FFHN's published Rust floor and exact stable toolchain to Rust `1.98`/`1.98.0`, moved HTML extraction to the public `htmlcut-core v13.1.0` contract, refreshed both Cargo graphs to their newest direct and transitive releases, removed unused workspace dependency declarations, and advanced the internal parser to `syn` 3.0.4, `cargo-nextest` to 0.9.143, and `cargo-llvm-cov` to 0.9.0.
...
- Stopped v10 operator-defined acquisition headers from crossing an HTTP origin boundary or being reattached later in the same redirect chain. Only the fixed `accept` and `user_agent` protocol fields may continue across origins, and HTTPS-to-HTTP redirects are refused.
- Prevented incomplete or partial HTTP bodies and non-representation success statuses such as `201` and `206` from becoming accepted observations, while accepting complete HTTP/1.0 connection-close framing and preserving typed failure evidence.
- Allowed empty but valid text observations to participate in condition and lifecycle event envelopes instead of being rejected as missing observation evidence.
- Preserved a nonzero process adapter's exit and stderr evidence when the adapter closes stdin without consuming the complete payload, instead of allowing the consequential broken-pipe writer failure to mask the authoritative process failure.
- Kept RustSec audit fail-closed for the lockfile-only `rkyv 0.7.46` advisory by permitting its exception only after an all-target, all-feature Cargo graph proves that the optional `rust_decimal` integration is unreachable; the audit fails before applying the exception if that proof changes or cannot run.
- Corrected the reports reference from HTMLCut v11 diagnostic-detail shapes to the currently pinned HTMLCut v13 interop surface.
