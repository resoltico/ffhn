...
pub(super) fn current_snapshot_reference(
    ...
    SnapshotReference {
        ...
        compare_digest_sha256: sha256_hex(compare_value.as_bytes()),
        ...
        compare_path: relative_artifact_path("snapshots/current/compare.txt"),
...
pub(super) fn stage_current_snapshot(
    ...
    write_snapshot_dir(&staged_dir, compare_value, outer_html, extraction_json)?;
    Ok((
        staged_dir,
        current_snapshot_reference(compare_value, outer_html, &captured_at),
    ))
}
...
pub(super) fn stage_history_snapshot(
    ...
    let reference = SnapshotReference {
        ...
        compare_digest_sha256: current.reference.compare_digest_sha256.clone(),
        ...
        compare_path: relative_artifact_path(format!(
            "snapshots/history/{snapshot_key}/compare.txt"
        )),
...
pub(super) fn write_snapshot_dir(
    ...
    let compare_value = normalize_line_endings(compare_value);
    let outer_html = normalize_line_endings(outer_html);
    write_exact_text(dir.join("compare.txt"), &compare_value)?;
...
fn history_snapshot_key(reference: &SnapshotReference) -> String {
    let compact_time = reference
        ...
        .collect::<String>()
        .to_ascii_lowercase();
    format!("{compact_time}-{}", &reference.compare_digest_sha256[..12])
}
