...
fn run_once_covers_config_invalid_lock_state_and_disabled_paths() {
    ...
    assert_eq!(
        report.failure_cause(),
        Some(RunFailureCause::IntegrityMismatch)
    );
    assert!(report.error_detail().is_some());
...
#[test]
fn run_once_reports_unavailable_target_paths_as_structured_failures() {
    let temp = tempdir().expect("tempdir");
    let paths = TargetPaths::new(temp.path(), "demo");
    std::fs::create_dir_all(paths.target_file()).expect("target file directory");

    let report = run_once(&paths).expect("target unavailable run");
    assert_eq!(report.run_outcome(), RunOutcome::FailedPermanent);
    assert_eq!(
        report.failure_cause(),
        Some(RunFailureCause::TargetUnavailable)
    );
    assert!(report.error_detail().is_some());
}
...
fn live_run_holds_the_exclusive_lock_until_the_run_finishes() {
    ...
    assert_eq!(
        second_report.failure_cause(),
        Some(RunFailureCause::LockUnavailable)
    );
    assert_eq!(second_report.run_outcome(), RunOutcome::FailedTransient);
    assert_eq!(
        second_report.baseline_phase_before_run(),
        BaselinePhase::HasBaseline
    );
    assert_eq!(
        second_report.baseline_phase_after_run(),
        BaselinePhase::HasBaseline
    );
    assert_eq!(
        second_report.previous_compare_digest_sha256(),
        Some(sha256_hex("After".as_bytes()).as_str())
    );
    assert_eq!(first_report.run_outcome(), RunOutcome::Changed);
}
...
fn run_once_reports_structured_persist_failures_for_live_disabled_fetch_extraction_and_success_paths()
 {
    ...
    assert!(!disabled_report.persist().committed_state());
    ...
    assert!(!fetch_report.persist().committed_state());
    ...
    assert_eq!(
        extraction_report.failure_cause(),
        Some(RunFailureCause::PersistError)
    );
    ...
    assert!(!extraction_report.persist().committed_state());
    ...
    assert!(matches!(
        success_report.body(),
        crate::RunBodyView::Reportable(_)
    ));
    assert!(success_report.current_compare_digest_sha256().is_some());
    assert!(!success_report.persist().committed_state());
...
fn run_once_stamps_run_finished_at_after_notification_delivery() {
    ...
    target.notification_endpoints = vec![notification_endpoint(
        "delay",
        "/bin/sh",
        vec!["-c", "cat >/dev/null; sleep 0.2"],
        1_000,
    )];
    target.notification_routes = vec![notification_route(
        "delay",
        vec![RunOutcome::Initialized],
        "/bin/sh",
        vec!["-c", "cat >/dev/null; sleep 0.2"],
        1_000,
    )];
    ...
    assert!(notifications[0].duration_ms() >= 150);
...
fn run_once_covers_fetch_and_extraction_failures() {
    ...
    assert_eq!(
        report.failure_cause(),
        Some(RunFailureCause::SelectionAmbiguousMatch)
    );
    ...
    assert_eq!(
        report.failure_cause(),
        Some(RunFailureCause::SelectionNoMatch)
    );
    assert!(paths.last_run_file().is_file());
}
...
fn run_once_initializes_then_detects_unchanged_and_changed_content() {
    ...
    assert!(paths.last_run_file().is_file());
    let last_run: crate::LastRunSnapshot = read_json(&paths.last_run_file()).expect("last run");
    assert!(report.persist().wrote_last_run());
    assert!(!last_run.run_report().persist().wrote_last_run());
    assert_eq!(last_run.run_report().run_outcome(), report.run_outcome());
    assert_eq!(
        last_run.run_report().failure_cause(),
        report.failure_cause()
    );
    assert_eq!(
