...
impl GateReporter {
    ...
    pub(crate) fn new(
        ...
        let _ = reporter.emit("run", "started", None, None, None, None);
        Ok(reporter)
    }
    ...
    pub(crate) fn run_spec(&mut self, repo_root: &Path, spec: &CommandSpec) -> DynResult<()> {
        let _ = self.emit(&spec.step_id, "started", None, None, None, None);
        ...
        let verbose = streams_success_output(self.options.format, self.options.verbosity);
    ...
    pub(crate) fn run_operation<T>(
        ...
        let _ = self.emit(step, "started", None, None, None, None);
        let started = Instant::now();
        match operation() {
            Ok(value) => {
                let _ = self.emit(
    ...
    pub(crate) fn finish(mut self, success: bool) -> DynResult<()> {
        self.failed = terminal_failure(self.failed, success);
    ...
    fn emit_warnings(&self, step: &str, stdout_path: &Path, stderr_path: &Path) -> DynResult<()> {
        ...
        {
            let _ = self.emit(step, "warning", None, None, Some(&line), None);
        }
        Ok(())
    }
    ...
    fn emit_failure_tail(
        ...
        let _ = self.emit(step, "diagnostic", None, None, Some(&tail), None);
        Ok(())
    }
    ...
    fn emit(
        ...
        let rendered = match self.options.format {
            ...
            GateOutputFormat::Json => serde_json::to_string(&GateEvent {
                schema: SCHEMA,
                gate: self.gate_id,
                step,
                state,
...
const fn streams_success_output(format: GateOutputFormat, verbosity: GateVerbosity) -> bool {
    matches!(format, GateOutputFormat::Human) && matches!(verbosity, GateVerbosity::Verbose)
}

const fn terminal_failure(observed_failure: bool, requested_success: bool) -> bool {
    observed_failure || !requested_success
}
    ...
    R: Read + Send + 'static,
{
    thread::spawn(move || {
        ...
        let live: Option<Box<dyn Write>> = if verbose {
            if stderr {
...
fn render_human_event(
    ...
    let log_dir = log_dir
        ...
        .unwrap_or_default();
    format!("[{state}] {step}{elapsed}{status}{message}{log_dir}")
}
