...
Both runners use `Swatinem/rust-cache` to persist the Cargo registry and incremental build artifacts across runs; without that cache, a cold Windows Rust build takes 15-20 minutes. Because that cache can also restore `~/.cargo/bin`, each CI job immediately reasserts the pinned Rust toolchain and any required QA-tool install after cache restore so cached proxy or binary state cannot override the canonical [../tooling/rust-tooling.env](../tooling/rust-tooling.env) manifest. CI, the contributor devcontainer, and local bootstrap all converge toolchains and Cargo QA tools through that same [../scripts/bootstrap-rust-tools.sh](../scripts/bootstrap-rust-tools.sh) entrypoint.
